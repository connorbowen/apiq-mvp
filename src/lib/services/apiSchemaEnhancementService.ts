/**
 * API Schema Enhancement Service
 * 
 * Centralized service for enhancing OpenAPI schemas with natural language understanding capabilities.
 * 
 ...
 * - API Explorer
 * 
 * Now uses AI for intelligent schema enhancement with fallback to rules-based patterns
 */

import { NaturalLanguageParameterExtractor, ParameterValueExtractionResult } from './naturalLanguageParameterExtractor';
...
export class ApiSchemaEnhancementService {
  /**
   * Enhance parameters from OpenAPI operation with natural language mappings
   */
  static async enhanceApiSchemas(
    operation: any, 
    openaiService?: any,
  ...
  private static async generateNaturalLanguageMappings(
    ...
    try {
      const parameterExtractor = new NaturalLanguageParameterExtractor(openaiService);
      
      const mappings = await parameterExtractor.generateParameterMappings(
        param,
        endpointContext || '',
        ''
      );
   ...
   */
  static async extractParameterValues(
    message: string,
    ...
    try {
      // Use AI-powered parameter value extraction
      const parameterExtractor = new NaturalLanguageParameterExtractor(openaiService);
      
      const extractionResult = await parameterExtractor.extractParameterValues(
        message,
        endpoint,
        context
      );

      logInfo('AI parameter value extraction completed', {
      ...
      logError('AI parameter value extraction failed, falling back to rules', error as Error, {
        message,
        endpointId: endpoint.id
      });
...
Return only a JSON object in this format:
{
  ...
  static async enhanceEndpoint(
    ...
    const enhancedParams = await ApiSchemaEnhancementService.enhanceApiSchemas(endpoint, openaiService, endpointContext);
