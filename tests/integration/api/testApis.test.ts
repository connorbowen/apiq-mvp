    ...
    TEST_APIS.forEach((api: TestApiConfig) => {
      it(`should reach base URL for ${api.name}`, async () => {
        try {
          ...
          // For APIs that don't support GET on base URL, that's okay
          // We just want to ensure the domain is reachable
          if (axios.isAxiosError(error)) {
            expect(error.code).not.toBe('ENOTFOUND'); // DNS resolution failed
            expect(error.code).not.toBe('ECONNREFUSED'); // Connection refused
          }
        }
      }, 10000);
    });
