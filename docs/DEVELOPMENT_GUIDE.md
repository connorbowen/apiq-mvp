...
## Canonical Prisma Client Usage

All code (API routes, helpers, and tests) must import the Prisma client from `lib/database/client.ts`. Do not instantiate new PrismaClient instances or use custom test helpers (such as `getTestPrismaClient`).

- The canonical import is:
  ```ts
  import { prisma } from '@/lib/database/client';
  ```
- The test runner sets `DATABASE_URL` for test isolation. All code paths (including tests) use the same Prisma instance and database connection.
- This ensures reliability, prevents subtle bugs, and is required by project rules.

**Legacy helpers such as `getTestPrismaClient` are now removed.**
